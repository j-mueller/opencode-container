...
```bash
nix run github:j-mueller/opencode-container/refs/tags/latest
```
...
```bash
podman run -it ghcr.io/j-mueller/opencode-container:latest
```
...
podman run --rm -it \
  ...
  ghcr.io/j-mueller/opencode-container:latest \
  opencode
```
